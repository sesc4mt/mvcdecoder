 ...
 *
 * Contains:
 *    Interface for sequence layer functions.
 *
 */
...
typedef struct _sequence_s {
  ...
  int redundantPicConcealmentCnt;
  mbAttributes_s *mbData_PP; /* Primary picture */
  frmBuf_s *recoBuf_PP;      /* Primary picture */
  mbAttributes_s *mbData_RP; /* Redundant picture */
  frmBuf_s *recoBuf_RP;      /* Redundant picture */
#endif

  /* These are for avcdDecodeOneFrame function */
  int streamPos;
  int decResult;
...
#ifdef REDUNDANT_PIC_CONCEALMENT
  errorConcealment_s *errConcealment_PP;
  errorConcealment_s *errConcealment_RP;
#endif

  /* Indicates whether to conceal frames (1) or generate non-existing frames (0) */
  int conceal;

  /* Number of pictures that were lost immediately before current picture */
  int numPrevPicsLost;
  ...
  dispBuf_s *displayBuffer;		 // Buffer for interlacing and output
