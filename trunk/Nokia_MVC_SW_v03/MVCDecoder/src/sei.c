...
sceneInfo_s *seiSceneOpen()
{
  ...
  scene = (sceneInfo_s *)nccMalloc(sizeof(sceneInfo_s));

  if (scene != NULL)
    memset(scene, 0, sizeof(sceneInfo_s));
...
int seiSceneInfo(sceneInfo_s *scene, bitbuffer_s *bitbuf)
{
  unsigned int  sceneInfoPresentFlag;
  int           retCode;

  /* scene_info_present_flag */
  if ((retCode = u_n(bitbuf, 1, &sceneInfoPresentFlag)) < 0) 
    return retCode;

  if (sceneInfoPresentFlag) {
    if ((retCode = ue_v(bitbuf, &scene->sceneId, 65535)) < 0)
      return retCode;
    if ((retCode = ue_v(bitbuf, &scene->sceneTransitionType, SCN_TRANS_TYPE_MAX)) < 0)
      return retCode;
    if (scene->sceneTransitionType > SCN_TRANS_TYPE_UNSPECIFY_COLOR) {
      if ((retCode = ue_v(bitbuf, &scene->secondSceneId, 65535)) < 0)
        return retCode;
    }
  }
