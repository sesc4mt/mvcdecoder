...
typedef struct _reorderCommand_s {
  int idc;
  int absDiffPicNumMinus1;
  int longTermPicNum;
} reorderCommand_s;


typedef struct _dpbBuf_s
{
  int frameNum;                       // frame number for short term reference
  int prevRefFrameNum;                // frame number of last reference frame
  int maxFrameNum;                    // max value current frame number can have

  int picHasMMCO5; /* the previous decoded picture in decoding order included a memory_management_control_operation equal to 5 */
  int prevPicHasMMCO5; /* the previous decoded picture in decoding order included a memory_management_control_operation equal to 5 */
  unsigned int frameNumOffset;
  unsigned int prevFrameNumOffset;

  int maxNumRefFrms;                  // max number of reference frames allowed
  int numRefFrms;                     // number of reference frames available
  int numShorts;                      // number of short-term reference frames
  int numLongs;                       // number of long-term reference frames
  int actualNumRefFrms;

  int idxOldest;                      // index to the oldest short term ref
  int ltrIdr;                         // indicate IDR will become a long term ref

  int numActiveRef;
  int maxNumRefFrmsMVC;               // The size of the reference picture list for MVC stream creation
  int reorderRefPicList;              // Flag to indicate a reordering of the Reference Picture List
  int numRefSameView;

  refFrmBuf_s *refFrmArr;             // The physical memory is allocated here
  refFrmBuf_s **refPicList0;          // The refPicList0
  refFrmBuf_s **refPicListNormalOrder;// Reference frame pointer array in normal order

  reorderCommand_s *reorderCmds;
  MMCO_s *mmcoList;
