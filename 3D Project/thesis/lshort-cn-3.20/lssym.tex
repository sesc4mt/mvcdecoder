...
% $Id: lssym.tex,v 1.2 2002/05/26 22:44:33 zuohuijun Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
\ifx\noAMS\relax 
使用表~\ref{AMSD}--\ref{AMSNBR}\footnote{这些表格来自~David\nbs Carlisle
的~\texttt{symbols.tex}，随后根据~Josef\nbs Tkadlec~的建议作了较大的改动。}
所列出的符号，必须事先安装~AMS~数学字库并且在文档的导言区加载宏包：~\pai{amssymb}。
如果你的系统中没有安装~AMS~宏包和数学字库，可去下述地址下载：\\
\texttt{CTAN:/tex-archive/macros/latex/required/amslatex}\fi

\begin{table}[!h]
%\caption{Math Mode Accents.}  \label{mathacc}
%
\caption{数学模式重音符} \label{mathacc}
%
\begin{symbols}{*4{cl}}
...
%
\centerline{\footnotesize $^a$使用宏包~\textsf{latexsym}~来得到这个符号}      
%
\end{table}
