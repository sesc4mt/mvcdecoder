 ...
 *
 * Contains:
 *    Standard C function wrappers etc.
 *
 */
...
void *nccMalloc(size_t size)
{
   ...
   if (p == NULL) {
      printf("nccMalloc: ERROR. Can't allocate %u bytes.\n", size);
      exit(1);
   }
...
void *nccCalloc(size_t num, size_t size)
{
   ...
   if (p == NULL) {
      printf("nccCalloc: ERROR. Can't allocate %u objects of size %u.\n", num, size);
      exit(1);
   }
...
void *nccRealloc(void *memblock, size_t size)
{
   ...
   if (p == NULL) {
      printf("nccRealloc: ERROR. Can't reallocate %u bytes.\n", size);
      exit(1);
   }
