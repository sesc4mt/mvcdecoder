 ...
 *
 * Contains:
 *    Interface for Nokia AVC decoder
 *
 */
...
#define AVCD_OK_RESOLUTION_AVAILABLE     3
...
/*
 * Structure for time stamps
 */
typedef struct _avcdTimingInfo_s
{
  unsigned long presentationTimeStampHi; /* 32 high bits of presentation time stamp */
  unsigned long presentationTimeStampLo; /* 32 low bits of presentation time stamp */
  unsigned long decodingTimeStampHi;     /* 32 high bits of decoding time stamp */
  unsigned long decodingTimeStampLo;     /* 32 low bits of decoding time stamp */
} avcdTimingInfo_s;


/*
 * Structure for decoded picture
 */
typedef struct _avcdYUVbuffer_s
{
  ...
  /* The following variables are from SPS's Video Usability Information */
  int aspectRatioNum;
  ...
  float frameRate; /* Derived */

  int imgPadding;

  /* From Picture Parameter Set */
  int frameQp;
  ...
  /* From Supplemental Enhancement Information */
