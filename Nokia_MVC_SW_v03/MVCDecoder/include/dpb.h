 ...
 *
 * Contains:
 *    Interface for Decoded Picture Buffer.
 *
 */
...
/* Set the number of views as two for now */
#define NUM_VIEWS 2

typedef struct _dpb_s {
  int size[NUM_VIEWS];

  /* Store reference frames for each view */
  frmBuf_s *buffers[DPB_MAX_SIZE][NUM_VIEWS];
  int fullness[NUM_VIEWS];
  int maxNumRefFrames;
  int numShortTermPics[NUM_VIEWS];
  int numLongTermPics[NUM_VIEWS];
  int maxLongTermFrameIdx[NUM_VIEWS];
} dpb_s;
...
frmBuf_s *dpbGetNextOutputPic(dpb_s *dpb, int *dpbHasIDRorMMCO5, int viewId);
