...
// the buffer has the extended boundaries, normally the extension is 16 pixels 
// for y, and 8 pixels for u and v
typedef struct _refFrmBuf_s 
{
  int16 picWidth;
  int16 picHeight;
  int16 xExt;           // horizontal extension at one side
  int16 yExt;           // vertical extension at one side

  rectRegion_s searchArea;

  // width and height of the extended buffer
  int16 yBufWidth;
  int16 yBufHeight;
  int16 cBufWidth;
  
  // y is upsampled, so y buffer size is twice as much in each direction,
  // u/v buffer size is half as much in each direction
  u_int8 *yBuf;       // y buffer with extended boundary areas
  u_int8 *uBuf;       // u buffer with extended boundary areas
  u_int8 *vBuf;       // v buffer with extended boundary areas
  u_int8 *y;          // point to the first actual y sample in the buffer
  u_int8 *u;          // point to the first actual u sample in the buffer
  u_int8 *v;          // point to the first actual u sample in the buffer

  u_int16 *seaYBuf;

  u_int16 *partSums;

  int frameNum;         // for a long-term ref, frameNum is "longTermFrmIdx"
  int picNum;           // for a long-term ref pic, picNum is "longTermPicNum"

  int forRef;           // Is this frame is used for reference?
  int forOutput;        // Is this frame is waiting for output?
  int isShortTerm;      // Is this frame used for short-term or long-term ref?

  int32 *channelDistortion;  // The expected channel distortion of the MBs

  int viewId;	// The id of the view this frame is from
} refFrmBuf_s;
