 ...
 * Contains:
 *
 *    Define structure containing encoder encoding parameters.
 *
 */


#ifndef _ENC_PARAMS_H_
#define _ENC_PARAMS_H_

#define MAX_SLICE_PARAMS    8


/* AVC encoder open parameters given to avceOpen */
// use only integer for all the members
typedef struct _encParams_s 
{
  unsigned level;

  int picWidth;
  int picHeight;
  long origFpsTimes1000;
  int maxNumRefFrms;
  int numReorderFrames;
  int modeFlags;

  int range;                // maximal motion search range
  int rangeMod;             // how should range be modified for other modes
  ...
  // use internal search window
  int useSearchWin;
  int srx;
  int sry;
#endif
  //// LOW_COMPLEX_PROF4
  int low_complex_prof4;
  ...
  int sgmType;
  int numSgmParams;
  int sgmParams[MAX_SLICE_PARAMS];

  // output frame cropping
  int cropOffsets[4];

  int scIntra;

  // these 3 variables are for tuning the rate distortion decision
  int tuneNonref;
  int tuneIntra;
  int tuneInter;

  // Parameters for bit rate control
  long brcBitRate;			// Compressed stream bit rate
  int brcBufSize;			// Encoder buffer size in bytes
  int numFrms;				// Number of frames to be encoded
  int brcdisableMBLevel; // Disable MB Level Rate Control
  ...
  // these are 2 syntax elements that could be changed by the user
  int log2_max_frame_num_minus4;

  int frameSkipEnabled;
  // depends on the time scale and how much reordering is performed
  int log2_max_pic_order_cnt_lsb_minus4;

  // Redundant picture parameters
  int redundantPicFlag;
  int redundantPicQpOffset;

  int rtp_flag;
} encParams_s;


#endif  // _ENC_PARAMS_H_
