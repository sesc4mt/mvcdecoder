 ...
 *
 * Contains:
 *    Interface for Nokia AVC encoder
 *
 */

#ifndef _AVCENCODER_H_
#define _AVCENCODER_H_

#include "encparams.h"


/* AVC encoder return codes */
#define AVCE_FRAME_SKIP_SLICE_OVERFLOW -2
#define AVCE_ERROR          -1
#define AVCE_OK             0
#define AVCE_FRAME_SKIPPED  1

typedef void avceEncoder_t;

typedef encParams_s avceOpenParams_s;


/* YUV buffer used to pass YUV data to and from encoder */
typedef struct _avceYUVbuffer_s 
{
  void *y;
  void *u;
  void *v;
...
/* Statistics returned by encder */
typedef struct _avceStatistics_s 
{
  double psnrY;
  double psnrU;
  double psnrV;
  ...
  int bufFullness;		// buffer occupancy at the end of a frame
  int numIntraMbs;		// number of macroblocks encoded as intra in this frame
} avceStatistics_s;
...
void avceClose(avceEncoder_t *encoder);
