 ...
 *
 * Contains:
 *    Interface for transform and quantization.
 *
 */
...
#define TRA_QUANT_BITS     15
#define TRA_QUANT_SCALE    (1<<TRA_QUANT_BITS)
#define TRA_QUANT_ROUND    (1<<(TRA_QUANT_BITS-1))
...
int32 traDiffSATD4x4(u_int8 orig[BLK_SIZE][MBK_SIZE],
                     u_int8 ref[BLK_SIZE][MBK_SIZE]);

int32 traDiffSATD4x4RetDc(u_int8 orig[BLK_SIZE][MBK_SIZE],
                          u_int8 ref[BLK_SIZE][MBK_SIZE], int *dc);

int traGetCoefCost(int *coef, int nonZero);
