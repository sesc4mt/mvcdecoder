...
typedef struct _sequence_s {
  encParams_s encPar;       // encoding parameters
  dpbBuf_s dpbBuf;          // decoded picture buffer & reference pointer list

  frmBuf_s       recoFrm;   // reconstructed frame buffer
  int lastIdrTr;            /* store the temp ref for the last Idr picture */
  int16 nextIdrPicId;         /* store the Id for the next Idr picture */
  int16 sliceGroupCycle;

  bitbuffer_s bitbuf;

  statSlice_s intraStat;
  statSlice_s interStat;
  statSlice_s frmStat;
  statNonSliceNal_s nonSliceStat;

  frmBuf_s recoBufMotField;
  ...
  rateCtrl_s bRC;
  mbAttributes_s mbData;    // MB state information after encoding
  slice_s      slice;            // slice coding object
  int *forcedIR;
  int  forcedIRNo;
