 ...
 *
 * Contains:
 *      Threshold definitions, frame structure and prototypes
 *      for Scene Cut Detector
 *
 */
...
/* If MS Visual C++ is used */
#ifdef _MSC_VER
   /* Disable warning generated in math.h */
   /* 'extern ' : ignored on left of 'double ' when no variable is declared */
...
#if defined MOVIDE || defined VCM_ENCODER
  // enable scene-cut detector
  #define SCENE_CUT_DETECTOR
#endif

#define QCIF_THRESHOLD_Y_HIGH 25344;
#define QCIF_THRESHOLD_Y_LOW  4000;
#define QCIF_THRESHOLD_UV     2600;

/*
 * If size of the frame is other than qcif, there is no need to change 
 * thresholds, because function can change thresholds for different framesizes.
 * QCIF_THRESHOLD_Y_HIGH is the threshold for certain scene cut. If the 
 * difference exceed it, there is definitely a scene cut.
 * QCIF_THRESHOLD_Y_LOW is the lower threshold for luminance, if the sum of
 * absolute difference of quantized histograms is lower than that, chrominance-
 * components will not be examined and there is no scene cut. If the framerate
 * is low, it may be better to raise that value little to avoid successive
 * scene cut detections.
 * QCIF_THRESHOLD_UV is the only threshold for chrominance-components, the
 * lower threshold is just 80% of that.
 */
