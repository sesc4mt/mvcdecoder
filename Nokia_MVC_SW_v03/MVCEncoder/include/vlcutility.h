 ...
 *
 * Contains:
 *    Interface for variable length coding, low-level operations.
 *
 */

#ifndef _VLC_UTILITY_H_
#define _VLC_UTILITY_H_

#include "globals.h"
#include "bitbuffer.h"

#define vlcGetMvBits(x, y)  ((int) uvlcBitsSigned[(x)+511]+(int) uvlcBitsSigned[(y)+511])

// bits on encode reference frame index, in truncated exp-Golomb code
#define vlcGetRefIndexBits(range, idx) (((range) <= 1) ? \
                                        (range) : uvlcBitsUnsigned[(idx)])

#define vlcuGetChromaNC(cbpDC, cbpC) ((cbpC == 0) ? ((cbpDC == 0) ? 0 : 1) : 2)
...
int  vlcuSendSignedUVLC(bitbuffer_s *bitbuf, int sValue);
...
extern const int8 uvlcBitsUnsigned[];
extern const int8 uvlcBitsSigned[];
extern const int8 numOfRefPar[7];

// macros defined to follow the standard specification
...
#define u_n(bitbuf, bits, len) vlcuSendFLC(bitbuf, bits, len)

#define CALCULATE_DELTA_QP(deltaQp, qp, prevQp)   {       \
  deltaQp = qp - prevQp;                                  \
  if (deltaQp < -(MAX_QP - MIN_QP + 1)/2)                 \
  deltaQp += MAX_QP - MIN_QP + 1;                       \
  if (deltaQp >= (MAX_QP - MIN_QP + 1)/2)                  \
  deltaQp -= MAX_QP - MIN_QP + 1;                       \
}

#endif  // _VLC_UTILITY_H_
