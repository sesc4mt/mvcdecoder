 ...
 *
 * Contains:
 *    Interface for variable length coding of syntax elements and coefficients.
 *
 */
...
typedef struct _vlcCoefBlkStat_s
{
  int levels[4*4];
  int8 runs[4*4];
  int  signs;
  int8 numTrailingZeros;
  int8 numTrailingOnes;
  int8 totalCoef;
  int8 totalZeros;
} vlcCoefBlkStat_s;

extern const int8 cbpCode[48][2];

int vlcSendIntraPred(bitbuffer_s *bitbuf, 
                     int8        *ipTab);
