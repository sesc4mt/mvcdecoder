 ...
 */
static int32 Diff_of_Q_Histograms(u_int8 *ptr1, u_int8 *ptr2, int32 fr_size);
...
#ifdef PERFORMANCE
static int histStart = 0, histStop = 0, diffStart = 0, diffStop = 0; 
extern int histTotal, diffTotal;
#endif
 ...
 *      ucurr
 *      vcurr
 *      yprev        previous frame
 *      uprev
 *      vprev
 *      sizex        width of a luminanceframe in pixels
 *      sizey        height of a luminanceframe in pixels
 *      subfactor    Chrominance subsampling factor (4:2:0 = 4, 4:2:2 = 2 etc.)
 *
 * Function:
 *      Examines if there is a scene cut between frames fr1 and fr2
 *      using local function Sum_of_Q_Histograms
 *
 * Returns:
 *      1, if there is a scene cut, 0 otherwise
 *
 * History:
 *      22.08.1995/Ari Hourunranta created
 *      06.11.1997/A H             - separate y, u, and v components
 */
int32 scdSceneCut(u_int8 *ycurr, u_int8 *ucurr, u_int8 *vcurr, u_int8 *yprev, u_int8 *uprev, u_int8 *vprev,
                  int32 sizex, int32 sizey, int32 subfactor, int *sumy, int *sumuv)
{
  int32 threshold_y1, threshold_y2, threshold_uv, sum_y = 0, sum_uv = 0;


  /* compare y-frames */
  sum_y = Diff_of_Q_Histograms(ycurr, yprev, sizex*sizey);
  *sumy = SUMY = sum_y;
  *sumuv = 0;

  /* there is definitely a scene cut if half of the colour is changed 
     the abs sum of difference of histograms is two times the size of
     the frames */
  threshold_y1 = QCIF_THRESHOLD_Y_HIGH;

  /* adapt the threshold */
  threshold_y1 = (int32)( (float32)threshold_y1 * ((float32)(sizex*sizey)/25344.0) );

  if (sum_y > threshold_y1) {
    return 1;
  }
  else {
    threshold_y2 = QCIF_THRESHOLD_Y_LOW;

    /* adapt the threshold */
    threshold_y2 = (int32)( (float32)threshold_y2 * ((float32)(sizex*sizey)/25344.0) );

    if (sum_y > threshold_y2) {

      /* compare v-frames */
      *sumuv = sum_uv = Diff_of_Q_Histograms(vcurr, vprev, sizey*sizex/subfactor);
      threshold_uv = QCIF_THRESHOLD_UV;


      /* adapt the threshold */
      threshold_uv = (int32)( (float32)threshold_uv * ( (float32)(sizex*sizey*4)/(float32)(subfactor*25344) ));

      /* if sum_y is in a upper half of the gray area, sum_uv can be lower
         than usually */

      if ((sum_uv > threshold_uv) || 
        ((sum_y > (threshold_y1 + threshold_y2)/2)&&
        (sum_uv > threshold_uv*4/5))) {
        ...
        /* compare u-frames */
        sum_uv = Diff_of_Q_Histograms(ucurr, uprev, sizey*sizex/subfactor); 


        /* if sum_y is in a upper half of the gray area, sum_uv can be 
           lower than usually */
...
/*
 *
 * Diff_of_Q_Histograms
 *
 * Parameters:
 *      ptr1         y-, u- or v-component of current frame
 *      ptr2         y-, u- or v-component of previous frame
 *      fr_size      sizex*sizey of components
 *
 * Function:
 *      Calculates histograms of 32 bin of both ptr's, calculates their
 *      difference and sums the absolute values of the difference
 *
 * Returns:
 *      sum of absolute difference of quantized (32 bin) histograms
 *
 * History:
 *      22.08.1995/Ari Hourunranta created
 */
...
{
  int16 i; 
  int32 j, s = 0, hist1[32], hist2[32];


  memset( hist1, 0, 32*sizeof(int) );
  memset( hist2, 0, 32*sizeof(int) );

#ifdef PERFORMANCE
  histStart = tmrGetTime();
#endif
...
#ifdef PERFORMANCE
  histStop = tmrGetTime();
  histTotal += histStop - histStart;

  diffStart = tmrGetTime();
#endif

  /* calculate the difference */
  for (i = 0; i < 32; i++)
    s += abs(hist2[i] - hist1[i]);

#ifdef PERFORMANCE
  diffStop = tmrGetTime();
  diffTotal += diffStop - diffStart;
#endif
