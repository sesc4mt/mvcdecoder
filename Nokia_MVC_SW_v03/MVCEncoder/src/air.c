 ...
 *
 * Contains:
 *    Functions for Adaptive Intra Refresh
 *
 */
...
int compute_AIRMAP(int *AIR_intraMap,int32 *channelDistortion,int AIR_s_numMb, int numMB)
{
  int i,j,num=0;
  for(i=0;i<numMB;i++)
  {
    for(j=0; j<i ; j++)
    {
      if(channelDistortion[i] > channelDistortion[j])
        AIR_intraMap[j]++;
      else
        AIR_intraMap[i]++;
    }
