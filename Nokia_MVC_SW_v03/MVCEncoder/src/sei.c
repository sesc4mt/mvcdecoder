 ...
 *
 * Contains:
 *    Functions for sending different SEI messages
 *
 */
...
/* 
 * sendBufferingPeriodSEIMessage	
 *
 * Parameters:
 *    bitbuf            the bitbuffer object
 *    bufsei			      Pointer to Buffering Period SEI Message structure      
 *	  hrd				        Pointer to HRD structure
 *    bitsNal           Pointer to counter accumulating NAL bits
 *
 * Function:
 *    Encode the Buffering Period SEI Message (payload_type + sei_payload)
 *
 * Returns:
 *    the number of bits being used for encoding the SEI. Please note that the stream header
 *    and the NAL header is not taken into account.
 */
int sendBufferingPeriodSEIMessage(bitbuffer_s *bitbuf,
                                  buffering_period_SEI_s *bufsei,
                                  hrd_parameters_s *hrd,
                                  int *bitsNal)
{

  int bits = 0, payloadBits=0;
  bitbuffer_s payloadBitBuf;

  bibOpen(&payloadBitBuf, 100); //100 is dummy here

  *bitsNal += vlcuSendNalUnitStartCodePrefix(bitbuf, 1);
  *bitsNal += vlcuSendNalHead(bitbuf, 0, SEI_NAL_TYPE);

  // Payload Type
  bits += u_n( bitbuf, 0, 8);
  
  //Put the payload bits to payloadBitBuf
  payloadBits+=	ue_v(&payloadBitBuf, bufsei->seq_parameter_set_id);
  
  payloadBits+= u_n(&payloadBitBuf, bufsei->initial_cpb_removal_delay, hrd->initial_cpb_removal_delay_length_minus1 + 1);
  payloadBits+= u_n(&payloadBitBuf, bufsei->initial_cpb_removal_delay_offset, hrd->initial_cpb_removal_delay_length_minus1 + 1); 
   
  payloadBits+= u_n(&payloadBitBuf, bufsei->initial_cpb_removal_delay, hrd->initial_cpb_removal_delay_length_minus1 + 1);
  payloadBits+= u_n(&payloadBitBuf, bufsei->initial_cpb_removal_delay_offset, hrd->initial_cpb_removal_delay_length_minus1 + 1); 

  if(payloadBits%8 != 0)
	  payloadBits+=u_n(&payloadBitBuf,1,1);
  while(payloadBits%8 != 0)
	  payloadBits+=u_n(&payloadBitBuf,0,1);

  //Payload Size
  bits += u_n( bitbuf, payloadBits/8, 8);
  
  //bibAppendBuffer(bitbuf, &payloadBitBuf);
  bits+=	ue_v(bitbuf, bufsei->seq_parameter_set_id);
  
  bits+= u_n(bitbuf, bufsei->initial_cpb_removal_delay, hrd->initial_cpb_removal_delay_length_minus1 + 1);
  bits+= u_n(bitbuf, bufsei->initial_cpb_removal_delay_offset, hrd->initial_cpb_removal_delay_length_minus1 + 1); 
...
/* 
 * sendPictureTimingSEIMessage	
 *
 * Parameters:
 *    bitbuf            the bitbuffer object
 *    picsei			Pointer to Picture Timing SEI Message structure      
 *	  hrd				Pointer to HRD structure
 *    bitsNal           Pointer to counter accumulating NAL bits
 *
 * Function:
 *    Encode the Picture Timing SEI Message (payload_type + sei_payload)
 *
 * Returns:
 *    the number of bits being used for encoding the SEI. Please note that the stream header
 *    and the NAL header is not taken into account.
 */
int sendPictureTimingSEIMessage(bitbuffer_s *bitbuf,
                                picture_timing_SEI_s *picsei,
                                hrd_parameters_s *hrd,
                                int *bitsNal)								  
{
  int bits = 0,  payloadBits=0;
  ...
  *bitsNal += vlcuSendNalHead(bitbuf, 0, SEI_NAL_TYPE);
  ...
  //Put the payload bits to payloadBitBuf
  payloadBits+=	u_n(&payloadBitBuf, picsei->cpb_removal_delay,hrd->cpb_removal_delay_length_minus1+1);
  payloadBits+=	u_n(&payloadBitBuf, picsei->dpb_output_delay,hrd->dpb_output_delay_length_minus1+1);
  ...
  //bibAppendBuffer(bitbuf, &payloadBitBuf);
  bits	+=	u_n(bitbuf, picsei->cpb_removal_delay,hrd->cpb_removal_delay_length_minus1+1);
  bits	+=	u_n(bitbuf, picsei->dpb_output_delay,hrd->dpb_output_delay_length_minus1+1);
...
int sendFillerPayloadSEIMessage(bitbuffer_s *bitbuf,int payloadSize, int *bitsNal)
{
  ...
  *bitsNal += vlcuSendNalHead(bitbuf, 0, SEI_NAL_TYPE);
  ...
  //Payload Size
  bits += u_n( bitbuf, payloadSize, 8);
  
  for(k=0;k<payloadSize;k++)
    bits  +=  u_n(bitbuf, 15,8);
